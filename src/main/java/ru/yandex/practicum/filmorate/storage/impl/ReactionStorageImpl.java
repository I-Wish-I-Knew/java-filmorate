...
@Repository
public class ReactionStorageImpl implements ReactionStorage {
    private final JdbcTemplate jdbcTemplate;

    @Autowired
    public ReactionStorageImpl(JdbcTemplate jdbcTemplate) {
    ...
    public List<Reaction> getReactions(Long id) {
        String sql = "SELECT * FROM reviews_reactions " +
                "WHERE review_id = ?";
        return jdbcTemplate.query(sql, this::mapRowToReactionOnReview, id);
    }
    ...
    public boolean containsReactionInStorage(Long reviewId, Long userId) {
        String sqlQuery = "SELECT count(*) FROM reviews_reactions " +
                "WHERE review_id = ? AND user_id = ?";
        long result = jdbcTemplate.queryForObject(sqlQuery, Integer.class, reviewId, userId);
        return result >= 1;
    }
